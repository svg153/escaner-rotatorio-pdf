...
class PDFMergerCore:
    ...
    def merge_pdfs(self, pdf_inputs: List[PDFInput], output_path: str) -> bool:
        ...
        try:
            ...
            for idx, pdf_input in enumerate(pdf_inputs, 1):
                ...
                pages = list(reversed(reader.pages)) if pdf_input.reverse else list(reader.pages)
    ...
    def process_pdf(self, input_pdf: str, output_pdf: str) -> bool:
        ...
        try:
            ...
            from utils.blank_detection import remove_blank_pages, check_blank_detection_availability
            from utils.deskew import auto_deskew_pdf, check_deskew_availability
            from utils.image_processing import process_pdf_images, check_image_processing_availability
            from utils.ocr import add_ocr_layer_to_pdf, check_ocr_availability
            from utils.metadata import add_metadata, add_text_watermark, add_page_numbers, check_metadata_availability
            ...
            if self.options.remove_blank:
                if check_blank_detection_availability():
                    ...
                    if self.options.verbose:
                        print("⚠ Omitiendo eliminación de páginas en blanco (dependencias no disponibles)")
            ...
            # 4. Image processing
            if any([self.options.enhance, self.options.denoise, self.options.binarize,
                   self.options.sharpen, self.options.despeckle, self.options.autocrop,
                ...
                if check_image_processing_availability():
                    ...
                    if self.options.verbose:
                        print("⚠ Omitiendo procesamiento de imágenes (dependencias no disponibles)")

            # 4b. Lossy recompression (convert pages to JPEG images at target DPI)
            if self.options.lossy:
                temp_lossy = self._create_temp_file()
                success = self._lossy_reencode(current_file, temp_lossy,
            ...
            if self.options.page_numbers:
                if check_metadata_availability():
                    ...
                    if self.options.verbose:
                        print("⚠ Omitiendo números de página (dependencias no disponibles)")
            
            # 9. Metadata
            if any([self.options.title, self.options.author, 
                   self.options.subject, self.options.keywords]):
                if check_metadata_availability():
    ...
    def merge_and_process(self, pdf_inputs: List[PDFInput], output_path: str,
        ...
        try:
            ...
            with pikepdf.open(input_pdf) as pdf:
                for i, page in enumerate(pdf.pages):
                    ...
                    if rotate != 0:
                        if self.options.verbose:
                            print(f"  Corrigiendo rotación de página {i+1}: {rotate}° → 0°")
                        page.Rotate = 0
    ...
    def _optimize_pdf(self, input_pdf: str, output_pdf: str, compress_level: int = 5) -> bool:
        """Optimize PDF size."""
        if not PIKEPDF_AVAILABLE:
            if self.options.verbose:
        ...
        try:
            ...
            if self.options.verbose:
                ...
                print(f"✓ Optimización completada (nuevo tamaño: {optimized_size:.2f} MB, reducción: {reduction:.1f}%)")
            return True
        except Exception as e:
            if self.options.verbose:
                print(f"⚠ Error al optimizar: {e}")
            return False
    def _lossy_reencode(self, input_pdf: str, output_pdf: str, dpi: int = 150, quality: int = 70) -> bool:
        ...
        try:
            ...
            if quality > 95: quality = 95  # Pillow advises <=95
            import pypdfium2 as pdfium
            from PIL import Image
            import img2pdf
            import io
            if self.options.verbose:
                print(f"Re-encodificando en modo lossy (dpi={dpi}, calidad={quality})...")
            ...
            layout_fun = None  # default: use image size
            with open(output_pdf, 'wb') as f_out:
                # Use default conversion; layout_fun None caused a callable error
                f_out.write(img2pdf.convert(page_images_bytes))
            if self.options.verbose:
                new_size = Path(output_pdf).stat().st_size / (1024*1024)
                print(f"✓ Re-encoding lossy completado (nuevo tamaño: {new_size:.2f} MB)")
            return True
        except Exception as e:
            if self.options.verbose:
