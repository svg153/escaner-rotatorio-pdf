...
def merge_pdfs():
    ...
    try:
        ...
        output_path = os.path.join(
            app.config["UPLOAD_FOLDER"], f"merged_{os.urandom(8).hex()}.pdf"
        )
        ...
        success = processor.merge_and_process(
            pdf_inputs, output_path, interleave=interleave
        )
...
def process_pdf():
    ...
    try:
        ...
        input_path = os.path.join(
            app.config["UPLOAD_FOLDER"], f"input_{secure_filename(file.filename)}"
        )
        file.save(input_path)

        # Create output path
        output_path = os.path.join(
            app.config["UPLOAD_FOLDER"], f"processed_{os.urandom(8).hex()}.pdf"
        )
