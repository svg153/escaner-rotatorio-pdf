...
class PDFMergerGUI:
    ...
    def create_widgets(self):
        ...
        tk.Button(btn_frame, text="Añadir PDFs", command=self.add_files).pack(side="left", padx=5)
        tk.Button(btn_frame, text="Quitar Seleccionado", command=self.remove_file).pack(side="left", padx=5)
        tk.Button(btn_frame, text="Limpiar Todo", command=self.clear_files).pack(side="left", padx=5)
        ...
        # Processing options
        options_frame = tk.LabelFrame(self.root, text="Opciones de Procesamiento", padx=10, pady=10)
        ...
        tk.Checkbutton(left_col, text="OCR (hacer buscable)", variable=self.ocr_var).pack(anchor="w")
        ...
        tk.Entry(ocr_lang_frame, textvariable=self.ocr_lang, width=10).pack(side="left", padx=5)
        
        # Image processing
        self.enhance_var = tk.BooleanVar()
        tk.Checkbutton(left_col, text="Mejorar contraste/brillo", variable=self.enhance_var).pack(anchor="w")
        
        self.denoise_var = tk.BooleanVar()
        tk.Checkbutton(left_col, text="Eliminar ruido", variable=self.denoise_var).pack(anchor="w")
        
        self.binarize_var = tk.BooleanVar()
        tk.Checkbutton(left_col, text="Blanco y negro", variable=self.binarize_var).pack(anchor="w")
        
        self.sharpen_var = tk.BooleanVar()
        tk.Checkbutton(left_col, text="Mejorar nitidez", variable=self.sharpen_var).pack(anchor="w")
        
        self.autocrop_var = tk.BooleanVar()
        tk.Checkbutton(left_col, text="Recortar bordes", variable=self.autocrop_var).pack(anchor="w")
        
        # Other options
        self.auto_deskew_var = tk.BooleanVar()
        tk.Checkbutton(right_col, text="Auto-enderezar", variable=self.auto_deskew_var).pack(anchor="w")
        
        self.remove_blank_var = tk.BooleanVar()
        tk.Checkbutton(right_col, text="Eliminar páginas en blanco", variable=self.remove_blank_var).pack(anchor="w")
        
        self.optimize_var = tk.BooleanVar()
        tk.Checkbutton(right_col, text="Optimizar tamaño", variable=self.optimize_var).pack(anchor="w")
        
        self.page_numbers_var = tk.BooleanVar()
        tk.Checkbutton(right_col, text="Añadir números de página", variable=self.page_numbers_var).pack(anchor="w")
        ...
        tk.Entry(watermark_frame, textvariable=self.watermark_var, width=15).pack(side="left", padx=5)
        ...
        tk.Entry(title_frame, textvariable=self.title_var).pack(side="left", fill="x", expand=True)
        ...
        tk.Entry(author_frame, textvariable=self.author_var).pack(side="left", fill="x", expand=True)
        
        # Profile selection
        profile_frame = tk.LabelFrame(self.root, text="Perfil Rápido", padx=10, pady=10)
        profile_frame.pack(padx=10, pady=5, fill="x")
        
        tk.Label(profile_frame, text="Cargar perfil predefinido:").pack(side="left", padx=5)
        ...
        tk.Button(profile_frame, text="Aplicar", command=self.apply_profile).pack(side="left", padx=5)
    ...
    def _process_thread(self, output_file):
        ...
        try:
            ...
            options = ProcessingOptions(
                ...
                watermark=self.watermark_var.get() if self.watermark_var.get() else None,
            ...
            if success:
                ...
                self.root.after(0, messagebox.showinfo, "Éxito", f"PDF generado:\n{output_file}")
            else:
                self.root.after(0, self._update_status, "✗ Error", "red")
                self.root.after(0, messagebox.showerror, "Error", "Error al procesar PDFs")
